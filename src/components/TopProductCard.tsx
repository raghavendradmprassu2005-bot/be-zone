...
const TopProductCard = ({ product }: TopProductCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-border/40 bg-card shadow-[0_4px_24px_-8px_rgba(0,0,0,0.10)] transition-shadow duration-300 hover:shadow-[0_16px_48px_-12px_rgba(0,0,0,0.20)]"
    >
      {/* Image */}
      <Link to={`/product/${product.id}`} className="relative block flex-shrink-0">
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-1.5">
          {discountPercent > 0 && (
            <Badge className="bg-foreground text-primary-foreground text-[11px] font-bold px-2.5 py-0.5 shadow-md">
              {discountPercent}% OFF
            </Badge>
          )}
          {product.tags.includes('hot-deal') && (
            <Badge className="bg-destructive text-destructive-foreground text-[11px] font-bold px-2.5 py-0.5 shadow-md">
              🔥 Hot
            </Badge>
          )}
        </div>
        ...
        <button
          ...
          className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full bg-background/90 shadow-md backdrop-blur-sm transition-all duration-200 hover:scale-110 active:scale-95"
      ...
      <div className="flex flex-1 flex-col p-4">
        {/* Category */}
        <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-secondary">
          {categoryEmoji[product.category]} {categoryLabel[product.category] || product.category}
        </p>

        {/* Product name */}
        <Link to={`/product/${product.id}`}>
          <h3 className="mb-2 font-display text-[15px] font-semibold leading-snug text-foreground transition-colors group-hover:text-secondary line-clamp-2">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mb-3 flex items-center gap-1.5">
          <div className="flex items-center gap-0.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                key={i}
                className={`h-3 w-3 ${i < Math.round(product.rating) ? 'fill-secondary text-secondary' : 'text-border'}`}
              />
            ))}
          </div>
        ...
        <div className="mt-auto flex flex-col gap-2">
          <div className="flex gap-2">
            <Button
              size="sm"
              variant="outline"
              className="flex-1 h-9 text-xs border-border/50 hover:bg-muted/50"
            ...
            <button
              ...
              className={`relative flex-1 h-9 overflow-hidden rounded-md px-3 text-xs font-semibold text-foreground transition-all duration-300 gradient-nebula hover:shadow-lg hover:shadow-secondary/30 hover:scale-[1.02] active:scale-95 ${ripple ? 'animate-pulse' : ''} ${isProcessingBuy ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
          ...
          <button
            ...
            className="flex h-9 w-full items-center justify-center gap-1.5 rounded-full bg-[#25D366] px-3 text-xs font-semibold text-white shadow-sm transition-all duration-200 hover:bg-[#1ebe57] hover:shadow-md hover:scale-[1.02] active:scale-95"
          >
